...
extern "C" void pipeline_norm(int N, void *f_void_arg, void *uex_void_arg, void *u_void_arg, void *err_void_arg, void * rnorm_void_arg)
{
  ...
  for (int  i2 = 1; (i2 <= N); i2 = (i2 + 1)) {
    double tmp1 = ( u[(i0-1)*(N+2)*(N+2) + (i1  )*(N+2) + (i2  )]
                  + u[(i0+1)*(N+2)*(N+2) + (i1  )*(N+2) + (i2  )]
                  + u[(i0  )*(N+2)*(N+2) + (i1-1)*(N+2) + (i2  )]
                  + u[(i0  )*(N+2)*(N+2) + (i1+1)*(N+2) + (i2  )]
                  + u[(i0  )*(N+2)*(N+2) + (i1  )*(N+2) + (i2-1)]
                  + u[(i0  )*(N+2)*(N+2) + (i1  )*(N+2) + (i2+1)]
                  - u[(i0  )*(N+2)*(N+2) + (i1  )*(N+2) + (i2  )] * 6.0
                  ) * invhh
                  - f[(i0  )*(N+2)*(N+2) + (i1  )*(N+2) + (i2  )];

    double tmp2 =   u[(i0  )*(N+2)*(N+2) + (i1  )*(N+2) + (i2  )]
                - uex[(i0  )*(N+2)*(N+2) + (i1  )*(N+2) + (i2  )];
