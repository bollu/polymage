...
def compare(map1, map2):
    ...
    keys1 = list(set([key for key in map1]))
    keys2 = list(set([key for key in map2]))
    is_equal = keys1 == keys2

    for key in map1:
        val1 = list(set(map1[key]))
        val2 = list(set(map2[key]))
        is_equal = is_equal and val1 == val2

    return is_equal

def test_liveness_computation():
